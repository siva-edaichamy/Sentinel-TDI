...
import s1_generate_raw as s1_generate_raw
import s2_transform_silver as s2_transform_silver
import s3_score_gold as s3_score_gold
import s4_build_platform as s4_build_platform
...
def _run_stage(
    ...
    if halt_on_failure and rows_out == 0 and stage_name not in ("s4_build_platform",):
        raise RuntimeError(
...
def run(dry_run: bool = False, env: str = "local", log_level: str = "INFO") -> dict:
    ...
    stages = [
        (s1_generate_raw,    "s1_generate_raw"),
        (s2_transform_silver,    "s2_transform_silver"),
        (s3_score_gold,      "s3_score_gold"),
        (s4_build_platform,  "s4_build_platform"),
        (s5_validate_pipeline,"s5_validate_pipeline"),
        (s6_report_analytics, "s6_report_analytics"),
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--from-stage", default=None,
                   choices=["s2_transform_silver", "s3_score_gold", "s4_build_platform",
                            "s5_validate_pipeline", "s6_report_analytics"],
...
if __name__ == "__main__":
    ...
    if args.from_stage:
        ...
        skip = {"s2_transform_silver": 1, "s3_score_gold": 2, "s4_build_platform": 3,
                "s5_validate_pipeline": 4, "s6_report_analytics": 5}
