"""
agent2_silver.py — Identity resolution and Silver-layer conformance.
...
PIPELINE_VERSION = os.getenv("PIPELINE_VERSION", "agent2_silver_v2")
...
def run(dry_run: bool = False, env: str = "local", log_level: str = "INFO") -> dict:
    ...
    logger.info("agent2_silver START | dry_run=%s env=%s", dry_run, env)
    ...
    try:
        ...
        logger.info(
            "agent2_silver DONE | rows_in=%d rows_out=%d unresolved=%d duration=%.2fs",
        ...
        duration = time.perf_counter() - t0
        logger.exception("agent2_silver FAILED: %s", exc)
        return {
            "status": "failure",
            "rows_in": rows_in,
            "rows_out": rows_out,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="agent2_silver — identity resolution and conformance")
