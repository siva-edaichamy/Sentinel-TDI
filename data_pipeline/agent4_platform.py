"""
agent4_platform.py — Platform artifact generation.
...
PIPELINE_VERSION = os.getenv("PIPELINE_VERSION", "agent4_platform_v1")
...
DAG structure:
    bronze_generate
        └── silver_resolve_[8 domains in parallel]
...
# Make agents importable from Airflow worker
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
...
def _check(result: dict, stage: str) -> None:
    """Raise AirflowException if agent returned failure."""
    if result["status"] != "success":
        raise AirflowException(
            f"{stage} failed: rows_in={result['rows_in']} "
            f"rows_out={result['rows_out']} artifacts={result['artifacts']}"
        )
...
@dag(
    ...
    catchup=False,
    owner_links={"airflow": "mailto:admin@corp.internal"},
...
def insider_threat_pipeline():
    ...
    @task()
    def bronze_generate():
        result = agent1_bronze.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "bronze_generate")
        return result

    @task()
    def silver_resolve_hris(bronze_result: dict):
        from agent2_silver import _transform_hris, _write_parquet, _gp_load, _now
        # Run just HRIS domain for parallel decomposition
        result = agent2_silver.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "silver_resolve_hris")
        return result

    @task()
    def silver_resolve_pacs(bronze_result: dict):
        result = agent2_silver.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "silver_resolve_pacs")
        return result

    @task()
    def silver_resolve_network(bronze_result: dict):
        result = agent2_silver.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "silver_resolve_network")
        return result

    @task()
    def silver_resolve_dlp(bronze_result: dict):
        result = agent2_silver.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "silver_resolve_dlp")
        return result

    @task()
    def silver_resolve_comms(bronze_result: dict):
        result = agent2_silver.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "silver_resolve_comms")
        return result

    @task()
    def silver_resolve_pai(bronze_result: dict):
        result = agent2_silver.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "silver_resolve_pai")
        return result

    @task()
    def silver_resolve_geo(bronze_result: dict):
        result = agent2_silver.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "silver_resolve_geo")
        return result

    @task()
    def silver_resolve_adjudication(bronze_result: dict):
        result = agent2_silver.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "silver_resolve_adjudication")
        return result

    @task()
    def gold_derive_features(silver_results: list[dict]):
        result = agent3_gold.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "gold_derive_features")
        return result

    @task()
    def gold_validate(gold_result: dict):
        result = agent5_validation.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "gold_validate")
        return result

    @task()
    def analytics_refresh(validation_result: dict):
        result = agent6_analytics.run(dry_run=False, env=ENV, log_level=LOG_LEVEL)
        _check(result, "analytics_refresh")
        return result

    # DAG wiring
    bronze = bronze_generate()

    silver_tasks = [
        silver_resolve_hris(bronze),
        silver_resolve_pacs(bronze),
        silver_resolve_network(bronze),
        silver_resolve_dlp(bronze),
        silver_resolve_comms(bronze),
        silver_resolve_pai(bronze),
        silver_resolve_geo(bronze),
        silver_resolve_adjudication(bronze),
    ]

    gold = gold_derive_features(silver_tasks)
    validation = gold_validate(gold)
    analytics_refresh(validation)
...
-- Train k-means on normalized feature vectors in employee_features
...
SELECT madlib.kmeans(
    ...
    {KMEANS_TOLERANCE},                  -- convergence tolerance
    '{GOLD_SCHEMA}.gd_kmeans_output'     -- output centroids table
);

-- Verify output
SELECT * FROM {GOLD_SCHEMA}.gd_kmeans_output;
"""
...
CREATE TABLE {GOLD_SCHEMA}.gd_scored AS
SELECT
    ef.employee_id,
    ef.window_end_date,
...
applications:
  - name: insider-threat-pipeline
    ...
    buildpacks:
      - python_buildpack
    command: python agents/agent0_orchestrator.py --env prod --log-level INFO
    env:
      RANDOM_SEED: "42"
      EMPLOYEE_COUNT: "500"
      TIMELINE_DAYS: "90"
...
def run(dry_run: bool = False, env: str = "local", log_level: str = "INFO") -> dict:
    ...
    logger.info("agent4_platform START | dry_run=%s env=%s", dry_run, env)
    ...
    try:
        ...
        logger.info("agent4_platform DONE | artifacts=%d duration=%.2fs", len(artifacts), duration)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="agent4_platform — platform artifact generation")
